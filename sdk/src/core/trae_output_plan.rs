...
fn build_output_files(
  ...
  for project in &prompt_projects {
    ...
    let content = if agents_registered {
      ...
      project.root_memory_prompt.as_ref().map(|rp| combine_global_with_content(global_mem, &rp.content))
    };

    if let Some(content) = content {
      let steering_dir = project_root_dir.join(".trae").join("steering");
      output_files.push(BaseOutputFileDeclarationDto {
        path: steering_dir.join(TRAE_STEERING_FILE).to_string_lossy().into_owned(),
      ...
      output_files.push(BaseOutputFileDeclarationDto {
        path: trae_cn_dir.join(TRAE_CN_USER_RULES_FILE).to_string_lossy().into_owned(),
        scope: Some(PROJECT_SCOPE.to_string()),
        content,
      });
    }
...
fn build_cleanup(workspace: &Workspace) -> CleanupDeclarationsDto {
  ...
  for project in get_project_output_projects(workspace) {
    ...
    delete.push(CleanupTargetDto {
      path: project_root_dir.join(".trae").join("steering").join(TRAE_STEERING_FILE).to_string_lossy().into_owned(),
    ...
    delete.push(CleanupTargetDto {
      path: project_root_dir.join(".trae-cn").join("user_rules").join(TRAE_CN_USER_RULES_FILE).to_string_lossy().into_owned(),
      kind: CleanupTargetKindDto::File,
      exclude_basenames: Vec::new(),
      protection_mode: None,
      scope: Some(PROJECT_SCOPE.to_string()),
