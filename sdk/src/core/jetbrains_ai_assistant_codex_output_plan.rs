...
pub fn collect_jetbrains_ai_assistant_codex_output_plan(context_json: &str) -> Result<String, CliError> {
...
pub fn build_jetbrains_ai_assistant_codex_output_plan(context: &CollectedInputContext) -> Result<BaseOutputPluginPlanDto, CliError> {
  let workspace = context.workspace.as_ref().ok_or_else(|| {
    CliError::ExecutionError("collectJetBrainsAIAssistantCodexOutputPlan requires collectedOutputContext.workspace".to_string())
  })?;
  Ok(BaseOutputPluginPlanDto { plugin_name: JB_PLUGIN_NAME.to_string(), output_files: Vec::new(), cleanup: build_cleanup(workspace) })
}
