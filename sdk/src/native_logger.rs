...
impl Logger {
  ...
  fn log_message(&self, level: LogLevel, message: Value, meta: Option<Value>) -> Option<LogRecord> {
...
mod napi_binding {
  ...
  fn normalize_message_payload(
    ...
    match message {
      Value::String(message_text) => {
        ...
        if meta_items.len() == 1 {
          ...
          if matches!(first, Value::Object(_)) {
            return (
              Value::String(message_text),
              Some(normalize_json_value(first)),
            );
          }
      ...
      other => (
        ...
        Some(normalize_json_value(other)),
      ),
    }
  }
  ...
  impl NapiLogger {
    ...
    pub fn emit_diagnostic(&self, level: String, diagnostic: Value) -> napi::Result<()> {
      ...
      self
        .inner
        .log_diagnostic(level, normalize_json_value(diagnostic));
      Ok(())
    }
  }
...
mod tests {
  ...
  fn test_build_copy_text_includes_expected_sections() {
    ...
    assert!(
      record
        .copy_text
        .contains(&"**Try this if needed**".to_string())
    );
