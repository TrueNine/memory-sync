...
import {AindexResolversInputCapability} from './input-aindex-resolvers'
...
describe('aindex input capability project config loading', () => {
  it('loads project.json5 using JSON5 features without any jsonc fallback', async () => {
    ...
    try {
      ...
      const result = await new AindexResolversInputCapability().collect(createContext(tempWorkspace, logger))
  ...
  it('collects app, ext, arch, and softwares projects with series-aware metadata', async () => {
    ...
    try {
      ...
      expect(projects.map(project => `${project.projectType}:${project.name}`)).toEqual([
        'app:project-a',
        'ext:plugin-a',
        'arch:system-a',
        'softwares:tool-a'
  ...
  it('fails fast when app, ext, arch, and softwares reuse the same project name', async () => {
    ...
    try {
      ...
      await expect(new AindexResolversInputCapability().collect(createContext(tempWorkspace, logger)))
        .rejects
