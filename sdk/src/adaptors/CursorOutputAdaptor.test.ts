import type {CommandPrompt, GlobalMemoryPrompt, OutputCleanContext, OutputWriteContext, RulePrompt, SkillPrompt} from './adaptor-core'
...
describe('cursorOutputAdaptor cleanup', () => {
  it('declares cleanup exclusions for built-in skills and lets core cleanup skip them', async () => {
    ...
    try {
      const plugin = new TestCursorOutputAdaptor(tempHomeDir)
  ...
  it('writes project-scoped commands, skills, mcp, and rules into workspace root through the synthetic workspace project', async () => {
    ...
    const plugin = new TestCursorOutputAdaptor(path.join(workspaceBase, 'home'))
    const ctx = {
      logger: createLogger('CursorOutputAdaptor', 'error'),
      fs,
      path,
      glob,
      dryRun: true,
