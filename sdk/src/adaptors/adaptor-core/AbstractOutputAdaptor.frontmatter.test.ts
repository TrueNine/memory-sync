...
import {AbstractOutputAdaptor} from './AbstractOutputAdaptor'

class TestFrontMatterOutputAdaptor extends AbstractOutputAdaptor {
  constructor(options?: ConstructorParameters<typeof AbstractOutputAdaptor>[1]) {
    super('TestFrontMatterOutputAdaptor', {
...
function createWriteContext(blankLineAfter?: boolean): OutputWriteContext {
  ...
  return {
    logger: createLogger('TestFrontMatterOutputAdaptor', 'error'),
    fs,
    path,
    glob: {} as never,
    dryRun: true,
...
describe('abstract output plugin front matter formatting', () => {
  it('adds a blank line after front matter by default for command/rule/subagent/skill outputs', async () => {
    const plugin = new TestFrontMatterOutputAdaptor()
