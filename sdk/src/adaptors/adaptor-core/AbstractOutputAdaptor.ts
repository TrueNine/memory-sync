...
/**
 * Options for configuring AbstractOutputAdaptor subclasses.
 */
export interface AbstractOutputAdaptorOptions {
  ...
  capabilities?: OutputAdaptorCapabilities
...
export abstract class AbstractOutputAdaptor extends AbstractAdaptor implements OutputAdaptor {
  ...
  protected constructor(name: string, options?: AbstractOutputAdaptorOptions) {
    super(name, AdaptorKind.Output, options?.dependsOn)
  ...
  private createCommandsConfig(config?: CommandOutputConfig): AbstractOutputAdaptor['commandsConfig'] {
    return {
  ...
  private createSubAgentsConfig(config?: SubAgentsOutputConfig): AbstractOutputAdaptor['subAgentsConfig'] {
    return {
  ...
  private createSkillsConfig(config?: SkillsOutputConfig): AbstractOutputAdaptor['skillsConfig'] {
    return {
  ...
  private buildInferredCapabilities(): OutputAdaptorCapabilities {
    const capabilities: OutputAdaptorCapabilities = {}
  ...
  private normalizeCapabilities(capabilities: OutputAdaptorCapabilities): OutputAdaptorCapabilities {
    const normalizedCapabilities: OutputAdaptorCapabilities = {}
  ...
  protected resolvePromptSourceProjectConfig(ctx: OutputAdaptorContext | OutputWriteContext): ProjectConfig | undefined {
  ...
  protected getConcreteProjects(ctx: OutputAdaptorContext | OutputWriteContext): Project[] {
  ...
  protected getProjectOutputProjects(ctx: OutputAdaptorContext | OutputWriteContext): Project[] {
    const projects = [...this.getConcreteProjects(ctx)]
  ...
  protected getProjectPromptOutputProjects(ctx: OutputAdaptorContext | OutputWriteContext): Project[] {
    return this.getProjectOutputProjects(ctx).filter(project => this.isProjectPromptOutputTarget(project))
  }

  protected getWorkspaceRootProject(ctx: OutputAdaptorContext | OutputWriteContext): Project | undefined {
    return ctx.collectedOutputContext.workspace.projects.find(project => project.isWorkspaceRootProject === true)
  }

  protected resolveProjectRootDir(ctx: OutputAdaptorContext | OutputWriteContext, project: Project): string | undefined {
  ...
  protected resolveProjectConfigDir(ctx: OutputAdaptorContext | OutputWriteContext, project: Project): string | undefined {
  ...
  protected resolveFrontMatterBlankLineAfter(ctx?: OutputAdaptorContext): boolean {
    if (!this.supportsBlankLineAfterFrontMatter) return true
    return ctx?.pluginOptions?.frontMatter?.blankLineAfter ?? true
  }

  protected buildMarkdownContent(content: string, frontMatter?: Record<string, unknown>, ctx?: OutputAdaptorContext): string {
    return buildMarkdownWithFrontMatter(frontMatter, content, {
      blankLineAfter: this.resolveFrontMatterBlankLineAfter(ctx)
    })
  }

  protected buildMarkdownContentWithRaw(content: string, frontMatter?: Record<string, unknown>, rawFrontMatter?: string, ctx?: OutputAdaptorContext): string {
  ...
  protected shouldSkipDueToPlugin(ctx: OutputWriteContext, precedingAdaptorName: string): boolean {
    const registeredPlugins = ctx.registeredAdaptorNames
    if (registeredPlugins == null) return false
    return registeredPlugins.includes(precedingAdaptorName)
  }
  ...
  protected getTopicScopeOverride(_ctx: OutputAdaptorContext | OutputWriteContext, _topic: OutputScopeTopic): OutputScopeSelection | undefined {
    void _ctx
    void _topic
    return void 0
  }
  ...
  protected buildRuleContent(rule: RulePrompt, ctx?: OutputAdaptorContext): string {
  ...
  protected async buildCommandContent(cmd: CommandPrompt, ctx?: OutputAdaptorContext): Promise<string> {
  ...
  protected buildSubAgentContent(agent: SubAgentPrompt, ctx?: OutputAdaptorContext): string {
  ...
  protected buildSkillMainContent(skill: SkillPrompt, ctx?: OutputAdaptorContext): string {
