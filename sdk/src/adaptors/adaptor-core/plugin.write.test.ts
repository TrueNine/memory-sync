import type {OutputAdaptor, OutputWriteContext} from './plugin'
...
function createOutputAdaptor(outputPath: string): OutputAdaptor {
  return {
...
describe('executeDeclarativeWriteOutputs', () => {
  ...
  it('removes blocking files and continues writing', async () => {
    ...
    const plugin = createOutputAdaptor(outputPath)
