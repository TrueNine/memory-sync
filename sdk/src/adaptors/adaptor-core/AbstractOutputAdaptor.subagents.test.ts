...
class TestSubAgentOutputAdaptor extends AbstractOutputAdaptor {
  constructor(options?: ConstructorParameters<typeof AbstractOutputAdaptor>[1]) {
    super('TestSubAgentOutputAdaptor', {
...
function createWriteContext(subAgents: readonly SubAgentPrompt[]): OutputWriteContext {
  ...
  return {
    logger: createLogger('TestSubAgentOutputAdaptor', 'error'),
    fs,
    path,
    glob: {} as never,
    dryRun: true,
...
describe('abstract output plugin subagent naming', () => {
  it('uses prefix-agent.ext by default', async () => {
    const plugin = new TestSubAgentOutputAdaptor()
