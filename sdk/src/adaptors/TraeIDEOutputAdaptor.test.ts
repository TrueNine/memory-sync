import type {OutputWriteContext, ProjectChildrenMemoryPrompt} from './adaptor-core'
import * as fs from 'node:fs'
import * as path from 'node:path'
import {describe, expect, it} from 'vitest'
import {createLogger, FilePathKind, PromptKind} from './adaptor-core'
import {TraeIDEOutputAdaptor} from './TraeIDEOutputAdaptor'
...
describe('traeIDEOutputAdaptor steering rule output', () => {
  it('emits project-relative glob and injects output-dir scope guard', async () => {
    const plugin = new TraeIDEOutputAdaptor()
    const workspaceBase = path.resolve('tmp/trae-plugin-test')
    const ctx = {
      logger: createLogger('TraeIDEOutputAdaptor', 'error'),
      fs,
      path,
      glob: {} as never,
      dryRun: true,
