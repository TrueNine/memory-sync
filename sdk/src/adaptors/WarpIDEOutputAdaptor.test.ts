import type {GlobalMemoryPrompt, OutputWriteContext, ProjectRootMemoryPrompt} from './adaptor-core'
import * as fs from 'node:fs'
import * as path from 'node:path'
import {describe, expect, it} from 'vitest'
import {createLogger, FilePathKind, PromptKind} from './adaptor-core'
import {WarpIDEOutputAdaptor} from './WarpIDEOutputAdaptor'
...
describe('warpIDEOutputAdaptor workspace prompt support', () => {
  it('writes the synthetic workspace root prompt to workspaceDir/WARP.md', async () => {
    ...
    const plugin = new WarpIDEOutputAdaptor()
    const ctx = {
      logger: createLogger('WarpIDEOutputAdaptor', 'error'),
      fs,
      path,
      glob: {} as never,
      dryRun: true,
