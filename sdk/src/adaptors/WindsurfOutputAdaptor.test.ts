import type {CommandPrompt, OutputCleanContext, OutputWriteContext, Project, RulePrompt, SkillPrompt} from './adaptor-core'
import * as fs from 'node:fs'
import * as os from 'node:os'
import * as path from 'node:path'
import {describe, expect, it} from 'vitest'
import {createLogger, FilePathKind, IgnoreFiles, PromptKind} from './adaptor-core'
import {WindsurfOutputAdaptor} from './WindsurfOutputAdaptor'

class TestWindsurfOutputAdaptor extends WindsurfOutputAdaptor {
...
function createWriteContext(
  ...
  return {
    logger: createLogger('WindsurfOutputAdaptor', 'error'),
    fs,
    path,
    glob: {} as never,
    dryRun: true,
...
describe('windsurfOutputAdaptor synthetic workspace project output', () => {
  it('writes workflows and skills to each real project when project scope is selected', async () => {
    ...
    const plugin = new WindsurfOutputAdaptor()
...
describe('windsurfOutputAdaptor cleanup', () => {
  it('declares cleanup for both .codeignore and legacy .codeiumignore', async () => {
    ...
    try {
      const plugin = new TestWindsurfOutputAdaptor(tempHomeDir)
