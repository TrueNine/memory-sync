import type {CommandPrompt, GlobalMemoryPrompt, OutputWriteContext, ProjectChildrenMemoryPrompt, ProjectRootMemoryPrompt, RulePrompt, SkillPrompt} from './adaptor-core'
import * as fs from 'node:fs'
import * as path from 'node:path'
import {describe, expect, it} from 'vitest'
import {createLogger, FilePathKind, PromptKind} from './adaptor-core'
import {QoderIDEPluginOutputAdaptor} from './QoderIDEPluginOutputAdaptor'
...
describe('qoderIDEPluginOutputAdaptor synthetic workspace project output', () => {
  it('writes workspace-root prompt, rules, commands, and skills through the synthetic workspace project', async () => {
    ...
    const plugin = new QoderIDEPluginOutputAdaptor()
    const ctx = {
      logger: createLogger('QoderIDEPluginOutputAdaptor', 'error'),
      fs,
      path,
      glob: {} as never,
      dryRun: true,
