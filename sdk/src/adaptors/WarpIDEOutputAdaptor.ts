...
import {AbstractOutputAdaptor, ADAPTOR_NAMES} from './adaptor-core'

const PROJECT_MEMORY_FILE = 'WARP.md'

export class WarpIDEOutputAdaptor extends AbstractOutputAdaptor {
  constructor() {
    super('WarpIDEOutputAdaptor', {
  ...
  override async declareOutputFiles(ctx: OutputWriteContext): Promise<OutputFileDeclaration[]> {
    ...
    const agentsRegistered = this.shouldSkipDueToPlugin(ctx, ADAPTOR_NAMES.AgentsOutput)
