import type {DiagnosticLines, LoggerDiagnosticInput, LoggerDiagnosticRecord} from './adaptors/adaptor-core'
...
function buildFileOperationAdvice(options: {
  ...
  if (options.blockingPath != null) {
    return {
      ...
      possibleFixes: [
        diagnosticLines(`A file is occupying a directory path required for "${options.path}".`),
        diagnosticLines('If that file came from an older tool or a mistaken manual edit, remove it and let tnmsc recreate the directory tree.')
      ]
    }
  }
  ...
  if (isWindowsDirectoryDeletePermissionDenied(options)) {
    return {
      ...
      possibleFixes: [
        diagnosticLines(`Use Resource Monitor or Process Explorer to find which process holds a handle under "${options.path}".`),
        diagnosticLines(`Make sure no shell, editor tab, or file watcher is currently opened inside "${options.path}" or one of its children.`),
        diagnosticLines(`If antivirus or cloud sync is scanning generated outputs, wait for it to release the directory or exclude this output path.`)
      ]
    }
  }

  return {
    exactFix: diagnosticLines(`Verify that "${options.path}" exists, has the expected type, and is accessible to tnmsc.`),
    possibleFixes: [
...
export function buildFileOperationDiagnostic(options: FileOperationDiagnosticOptions): LoggerDiagnosticInput {
  const {code, title, operation, targetKind, path, error, platform, exactFix, possibleFixes, details} = options
  const errorMessage = toErrorMessage(error)
  const blockingPath = targetKind === 'file' || targetKind === 'directory' ? resolveBlockingFilePath({path, targetKind, error}) : void 0
  ...
  return buildDiagnostic({
    code,
    title,
    rootCause: diagnosticLines(`Could not ${operation} the ${targetKind} at "${path}".`, `Error: ${errorMessage}`),
...
export function buildBatchFileOperationDiagnostic(options: BatchFileOperationDiagnosticOptions): LoggerDiagnosticInput {
  const {code, title, operation, targetKind, failures, exactFix, possibleFixes, details} = options
  const firstFailure = failures[0]
  const firstFailureLine
    = firstFailure == null ? 'No failing path details were captured.' : `First failure: "${firstFailure.path}" -> ${toErrorMessage(firstFailure.error)}`

  return buildDiagnostic({
    code,
    title,
    rootCause: diagnosticLines(`${failures.length} ${operation} operation(s) failed while handling ${targetKind}.`, firstFailureLine),
    exactFix: exactFix ?? diagnosticLines(`Fix the failing ${targetKind} path, then retry tnmsc.`),
...
export function buildConfigDiagnostic(options: ConfigDiagnosticOptions): LoggerDiagnosticInput {
  const {code, title, reason, configPath, exactFix, possibleFixes, details} = options

  return buildDiagnostic({
    code,
    title,
    rootCause: configPath == null ? reason : diagnosticLines(reason[0], ...reason.slice(1), `Config path: ${configPath}`),
    exactFix,
    possibleFixes,
    details: {
...
export function buildPathStateDiagnostic(options: PathStateDiagnosticOptions): LoggerDiagnosticInput {
  const {code, title, path, expectedKind, actualState, exactFix, possibleFixes, details} = options

  return buildDiagnostic({
    code,
    title,
    rootCause: diagnosticLines(`Expected a ${expectedKind} at "${path}".`, `Actual state: ${actualState}`),
    exactFix: exactFix ?? diagnosticLines(`Create or replace "${path}" with a valid ${expectedKind} before retrying tnmsc.`),
    possibleFixes: possibleFixes ?? [
...
export function buildPromptCompilerDiagnostic(options: PromptCompilerDiagnosticOptions): LoggerDiagnosticInput {
  const {code, title, diagnosticText, exactFix, possibleFixes, details} = options
  ...
  return buildDiagnostic({
    ...
    exactFix: exactFix ?? diagnosticLines('Fix the referenced prompt source or compiled file, then rerun tnmsc.'),
    possibleFixes: possibleFixes ?? [
...
export function buildProtectedDeletionDiagnostic(operation: string, violations: readonly ProtectedPathViolation[]): LoggerDiagnosticInput {
  ...
  return buildDiagnostic({
    ...
    rootCause: diagnosticLines(
      ...
      firstViolation != null ? `Example protected path: ${firstViolation.protectedPath}` : 'No violation details were captured.'
    ),
    exactFix: diagnosticLines('Remove protected inputs or reserved workspace paths from the delete plan before running tnmsc again.'),
    possibleFixes: [
...
export function buildUnhandledExceptionDiagnostic(context: string, error: unknown): LoggerDiagnosticInput {
  ...
  return buildDiagnostic({
    ...
    rootCause: diagnosticLines(`An unhandled exception escaped the ${context} flow.`, `Error: ${errorMessage}`),
    exactFix: diagnosticLines('Inspect the failing code path, add the missing guard or validation, then retry the command.'),
    possibleFixes: [
...
export function partitionBufferedDiagnostics(diagnostics: readonly LoggerDiagnosticRecord[]): {
  warnings: PublicLoggerDiagnosticRecord[]
  errors: PublicLoggerDiagnosticRecord[]
} {
