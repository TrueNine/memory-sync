...
fn replace_source_mdx_extension(path: &str) -> Option<String> {
  path
    .strip_suffix(PRIMARY_SOURCE_MDX_EXTENSION)
...
/// Get log output from a CLI command.
///
/// Previously used the legacy Node bridge; now returns an empty log stream
/// until command logging is fully implemented in Rust.
#[tauri::command]
pub fn get_logs(_cwd: String, _command: String) -> Result<Vec<LogEntry>, String> {
  Ok(Vec::new())
}
...
fn collect_root_memory_prompt_stats(
  ...
  for (source_rel, _) in collect_root_memory_prompt_pairs(config) {
    for relative_path in
      std::iter::once(source_rel.clone()).chain(derive_english_source_rel(&source_rel).into_iter())
    {
...
mod tests {
  ...
  fn collect_project_series_category_files_scans_app_ext_and_arch() {
    ...
    std::fs::create_dir_all(app_dist.join("project-a")).expect("app dist dir should be created");
    std::fs::create_dir_all(ext_dist.join("plugin-a")).expect("ext dist dir should be created");
    std::fs::create_dir_all(arch_dist.join("system-a")).expect("arch dist dir should be created");
