    ...
    it('installs opencode outputs from dist content', async () => {
      ...
      await withPluginEnvironment(artifacts, fixture, async container => {
        ...
        assertDistContent(agent,
          ['mode: subagent', 'Review changes carefully.'],
          ['请仔细审查改动。'],
        )

        const skill = container.readFile(fixture.outputPaths.projectSkill)
        assertDistContent(skill,
          ['name: ship-it', 'description: Ship-it skill', 'English dist skill body'],
          ['中文技能内容'],
        )

        const mcp = container.readFile(fixture.outputPaths.projectSkillMcp)
        assertDistContent(mcp,
          ['"$schema": "https://opencode.ai/config.json"', '"opencode-rules@latest"', '"mcp": {}'],
          [],
        )
