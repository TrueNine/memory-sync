...
pub fn integration_tests_dir() -> PathBuf {
  cli_manifest_dir().join("integration-tests")
}
...
pub fn tnmsc_binary() -> PathBuf {
  PathBuf::from(env!("CARGO_BIN_EXE_tnmsc"))
}
...
pub fn run_tnmsc_with_env(args: &[&str], cwd: &Path, envs: &[(&str, &str)]) -> CommandResult {
  let mut command = Command::new(tnmsc_binary());
  command.args(args).current_dir(cwd);
  for (key, value) in envs {
    command.env(key, value);
  }

  command_output(&mut command, "tnmsc")
}
...
pub fn ensure_release_binary() {
  RELEASE_BINARY_BUILT.get_or_init(|| {
    let manifest_path = cli_manifest_dir().join("Cargo.toml");
    ...
    result.assert_success("cargo build --release for tnmsc");
  });
