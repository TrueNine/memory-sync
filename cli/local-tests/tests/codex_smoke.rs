...
/// Regression test: skill output directories should only contain SKILL.md.
///
/// Prior bug: resources, child docs, and mcp config files were rendered inside
/// each skill directory (e.g. act/act/, references/*), creating incorrect
/// nested structures and duplicate content.
///
/// Note: only checks plugins generated by `tnmsc install` (codex, opencode,
/// claude). droid (.factory/skills) is a separate plugin not emitted by
/// install; it has its own `skill_output_only_contains_skill_md` unit test.
#[test]
fn regression_skill_output_only_contains_skill_md() {
  assert_codex_plugin_enabled();
  ...
  for (label, skills_dir) in [
    ("codex", runner.cwd().join(".codex").join("skills")),
    ("opencode", runner.cwd().join(".opencode").join("skills")),
    ("claude", runner.cwd().join(".claude").join("skills")),
  ] {
    assert!(
      skills_dir.is_dir(),
      "{label} skills dir should exist: {}",
      skills_dir.display()
    );
    ...
    for entry in std::fs::read_dir(&skills_dir).unwrap().flatten() {
      ...
      assert_eq!(
        entries,
        vec!["SKILL.md"],
        "{label}/skills/{skill_name} should only contain SKILL.md, got: {:?}",
        entries
      );
    }
  }
