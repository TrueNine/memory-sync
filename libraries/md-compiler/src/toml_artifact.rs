...
fn order_entries(entries: Vec<(String, Value)>, field_order: Option<&[String]>) -> Vec<(String, Value)> {
    ...
    entries.sort_by(|(left_key, _), (right_key, _)| match (priority.get(left_key.as_str()), priority.get(right_key.as_str())) {
...
fn render_toml_section(
    ...
    let ordered_entries = order_entries(
        value.iter().map(|(key, value)| (key.clone(), value.clone())).collect(),
        field_order,
    );
    ...
    for (key, table_value) in table_entries {
        ...
        next_path.push(key);
        lines.extend(render_toml_section(&next_path, &table_value, field_order, true)?);
    }
    ...
    for (key, table_values) in array_table_entries {
        for table_value in table_values {
            ...
            lines.extend(render_toml_section(&next_path, &table_value, field_order, false)?);
        }
    }
...
pub fn build_toml_document(
    ...
    let normalized = normalize_value(value).ok_or_else(|| "TOML document root must be an object".to_string())?;
