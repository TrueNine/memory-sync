...
pub use mdx_to_md::{
  ExportMetadata, MdxGlobalScope, MdxToMdOptions, MdxToMdResult, mdx_to_md, mdx_to_md_with_metadata,
};
...
mod napi_binding {
  use super::{
    BuildPromptTomlArtifactOptions, BuildTomlDocumentOptions, EvaluationScope, MdxGlobalScope,
    MdxToMdOptions, build_prompt_toml_artifact, build_toml_document, mdx_to_md,
  ...
  pub fn mdx_to_md_with_scope(content: String, scope_json: String) -> napi::Result<String> {
    let scope: EvaluationScope =
      serde_json::from_str(&scope_json).map_err(|e| napi::Error::from_reason(e.to_string()))?;
  ...
  #[napi]
  pub fn compile_mdx_to_md(content: String, options_json: Option<String>) -> napi::Result<String> {
    ...
    let result = if options.extract_metadata {
      ...
      let compiled =
        mdx_to_md(&content, Some(options)).map_err(|e| napi::Error::from_reason(e.to_string()))?;
  ...
  pub fn build_toml_document_binding(
    ...
    let document: Value =
      serde_json::from_str(&document_json).map_err(|e| napi::Error::from_reason(e.to_string()))?;
