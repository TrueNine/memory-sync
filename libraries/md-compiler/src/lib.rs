...
mod napi_binding {
    ...
    use super::{mdx_to_md, MdxToMdOptions, EvaluationScope};
...
pub fn build_front_matter(front_matter_json: String) -> napi::Result<String> {
    let obj: Value =
        serde_json::from_str(&front_matter_json).map_err(|e| napi::Error::from_reason(e.to_string()))?;

    let map = match &obj {
        Value::Object(m) => m,
        _ => return Err(napi::Error::from_reason("frontMatter must be a JSON object")),
    };
...
pub fn transform_mdx_references_to_md(content: String) -> String {
    ...
    re.replace_all(&content, |caps: &regex_lite::Captures| {
        ...
        let url = &caps[4];
        let suffix = &caps[5];
        let transformed_url = if url.starts_with("http://") || url.starts_with("https://") || url.starts_with("//") {
            url.to_string()
        } else {
            url.replace(".mdx", ".md")
        };
