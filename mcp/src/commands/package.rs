...
fn copy_target_from_artifacts(
  ...
  let source = artifacts_dir
    .join(format!("mcp-binary-{}", target.suffix))
...
fn copy_target_from_local_build(target: &PackageTarget, profile: &str) -> Result<PathBuf, String> {
  ...
  if !source.is_file() {
    return Err(format!(
      "Missing local host binary at {}. Run cargo build --{} --manifest-path mcp/Cargo.toml first.",
      source.display(),
      profile
    ));
  }
...
fn package_root() -> PathBuf {
  std::env::var_os("TNMSM_NPM_PACKAGE_ROOT")
    .map(PathBuf::from)
    .unwrap_or_else(|| PathBuf::from(env!("CARGO_MANIFEST_DIR")))
}

fn workspace_root() -> PathBuf {
  if let Some(path) = std::env::var_os("TNMSM_WORKSPACE_ROOT") {
