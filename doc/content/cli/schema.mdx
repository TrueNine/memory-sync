...
Unknown keys are not ignored anymore. If `plugins` contains an unsupported key, `tnmsc` logs a config validation error, shows the supported key list, and exits.
...
> **Warning**
> Output plugins often write into tool-owned directories such as `.codex/` or `.claude/`. If a same-name file already exists where tnmsc needs a directory, that is treated as a conflict because the directory tree cannot be created normally.
>
> The runtime keeps moving by design: it warns, removes the blocking file, and then recreates the required directory path so the sync can finish.

For example, this now fails validation:
...
`foo` is rejected because it is not one of the supported plugin keys listed above.
