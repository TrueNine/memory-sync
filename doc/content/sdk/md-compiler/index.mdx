...
<Callout type="info">
  **包标识**: `@truenine/croessweave-sdk` (npm) / `tnmsd` (Rust crate)
...
MDX-Compiler 是 croessweave monorepo 中的 **Rust-first MDX/Markdown 编译与转换引擎**。它负责将 MDX 源码编译为纯 Markdown 文本，同时提供表达式求值、JSX 组件处理、导出元数据提取以及 TOML artifact 构建等核心能力。
...
- 支持从 CLI binary package (`@truenine/croessweave-cli-<suffix>`) 中提取 binding
...
```typescript
import { ToolPresets } from '@truenine/croessweave-sdk/globals'
...
```typescript
import { mdxToMd } from '@truenine/croessweave-sdk'
...
```typescript
import { mdxToMd, MdxToMdOptions } from '@truenine/croessweave-sdk'
...
```typescript
import { mdxToMd, buildPromptTomlArtifact } from '@truenine/croessweave-sdk'
...
以下是 `@truenine/croessweave-sdk` 的完整公共类型导出清单：
