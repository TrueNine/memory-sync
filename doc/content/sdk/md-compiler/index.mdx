...
<Callout type="info">
  **包标识**: `@truenine/md-compiler` (npm) / `tnmsc-md-compiler` (Rust crate)  
  **源码位置**: `libraries/md-compiler/`
  **导出入口**: `src/index.ts`
</Callout>
...
1. **Native Binding 层（Rust/NAPI）**：核心编译逻辑由 Rust crate `tnmsc-md-compiler` 实现，通过 NAPI-RS 暴露给 Node.js。公开包入口默认走这条路径。
...
<Callout type="info">
  MDX-Compiler 通过 workspace 依赖使用 `tnmsc-logger` 库进行编译过程中的日志输出。
</Callout>
...
```typescript
import { ToolPresets } from '@truenine/md-compiler/globals'
...
```typescript
import { mdxToMd, MdxToMdOptions } from '@truenine/md-compiler'
...
```typescript
import { mdxToMd, buildPromptTomlArtifact } from '@truenine/md-compiler'
...
以下是 `@truenine/md-compiler` 的完整公共类型导出清单：
