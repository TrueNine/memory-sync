...
<Callout type="info">
  **包标识**: `@truenine/memory-sync-sdk` (npm) / `tnmsc` (Rust crate)  
...
1. **Native Binding 层（Rust/NAPI）**：核心编译逻辑由 Rust crate `tnmsc` 实现，通过 NAPI-RS 暴露给 Node.js。公开包入口默认走这条路径。
...
<Callout type="info">
  MDX-Compiler 通过 workspace 依赖使用 `tnmsc` 库进行编译过程中的日志输出。
</Callout>
