...
<Callout type="info">
**包名**: `@truenine/memory-sync-sdk` &nbsp;|&nbsp; **Rust Crate**: `tnmsc` &nbsp;|&nbsp; **Rust / 包位置**: `sdk/src/sdk/script-runtime/` &nbsp;|&nbsp; **TypeScript 事实来源**: `sdk/src/sdk/src/sdk/script-runtime/`
</Callout>
...
```typescript
// proxy.ts - 函数式定义
import {defineProxy} from '@truenine/memory-sync-sdk'
...
```typescript
// proxy.ts - 对象式定义
import {defineProxy, type ProxyDefinition} from '@truenine/memory-sync-sdk'
import {join} from 'node:path'
...
Script-Runtime 现在与 Logger、MDX-Compiler 共用 `sdk/src/core/native-binding-loader.ts`。它的 TypeScript 事实来源已经迁到 `sdk/src/sdk/src/sdk/script-runtime/`，而 `sdk/src/sdk/src/sdk/script-runtime/index.ts` 只保留 wrapper 角色；真正的实现只保留 Script-Runtime 自己必须关心的部分：
...
```typescript
// project/proxy.ts
import {defineProxy, type ProxyRouteHandler} from '@truenine/memory-sync-sdk'
...
```typescript
import {loadProxyModule, getProxyModuleConfig} from '@truenine/memory-sync-sdk'
...
```typescript
import {validatePublicPath} from '@truenine/memory-sync-sdk'
...
```typescript
import {resolvePublicPath} from '@truenine/memory-sync-sdk'
...
```typescript
import {resolvePublicPath, loadProxyModule, getProxyModuleConfig} from '@truenine/memory-sync-sdk'
...
```typescript
import {resolvePublicPath, loadProxyModule} from '@truenine/memory-sync-sdk'
...
graph TB
    ...
    subgraph ScriptRuntimeAPI["@truenine/memory-sync-sdk"]
        direction TB
...
- **Rust / 包位置**: `sdk/src/sdk/script-runtime/`
- **TypeScript 事实来源**: `sdk/src/sdk/src/sdk/script-runtime/`
- **主入口**: `sdk/src/sdk/src/sdk/script-runtime/index.ts`
- **类型定义**: `sdk/src/sdk/src/sdk/script-runtime/types.ts`
- **核心运行时**: `sdk/src/sdk/src/sdk/script-runtime/runtime-core.ts`
- **Worker 处理器**: `sdk/src/sdk/src/sdk/script-runtime/resolve-proxy-worker.ts`
