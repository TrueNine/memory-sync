...
<Callout type="info">
**包名**: `@truenine/script-runtime` &nbsp;|&nbsp; **Rust Crate**: `tnmsc-script-runtime` &nbsp;|&nbsp; **Rust / 包位置**: `libraries/script-runtime/` &nbsp;|&nbsp; **TypeScript 事实来源**: `sdk/src/libraries/script-runtime/`
</Callout>
...
```typescript
// proxy.ts - 函数式定义
import {defineProxy} from '@truenine/script-runtime'
...
```typescript
// proxy.ts - 对象式定义
import {defineProxy, type ProxyDefinition} from '@truenine/script-runtime'
import {join} from 'node:path'
...
Script-Runtime 现在与 Logger、MDX-Compiler 共用 `sdk/src/core/native-binding-loader.ts`。它的 TypeScript 事实来源已经迁到 `sdk/src/libraries/script-runtime/`，而 `libraries/script-runtime/src/index.ts` 只保留 wrapper 角色；真正的实现只保留 Script-Runtime 自己必须关心的部分：
...
```typescript
// project/proxy.ts
import {defineProxy, type ProxyRouteHandler} from '@truenine/script-runtime'
...
```typescript
import {loadProxyModule, getProxyModuleConfig} from '@truenine/script-runtime'
...
```typescript
import {validatePublicPath} from '@truenine/script-runtime'
...
```typescript
import {resolvePublicPath, loadProxyModule, getProxyModuleConfig} from '@truenine/script-runtime'
...
```typescript
import {resolvePublicPath, loadProxyModule} from '@truenine/script-runtime'
...
graph TB
    ...
    subgraph ScriptRuntimeAPI["@truenine/script-runtime"]
        direction TB
    ...
    subgraph NativeLayer["Rust Native Layer (tnmsc-script-runtime)"]
        direction TB
...
- **Rust / 包位置**: `libraries/script-runtime/`
- **TypeScript 事实来源**: `sdk/src/libraries/script-runtime/`
...
- **Rust Native**: `libraries/script-runtime/src/lib.rs`
