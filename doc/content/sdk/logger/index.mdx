...
<Callout type="info">
**包信息**
- **npm 包名**: `@truenine/memory-sync-sdk`
- **Rust crate**: `tnmsc`
- **Rust 源码位置**: `sdk/src/native_logger.rs`
- **TypeScript 事实来源**: `sdk/src/sdk/logger.ts`
- **发布壳层**: `sdk/src/sdk/logger.ts`
- **核心文件**: `sdk/src/native_logger.rs` (Rust 核心) · `sdk/src/sdk/logger.ts` (TypeScript facade)
</Callout>
...
Logger 的 TypeScript facade 已迁到 `sdk/src/sdk/logger.ts`，`sdk/src/sdk/logger.ts` 现在只是一个薄 re-export wrapper。SDK 内联实现不再复制平台检测、本地候选路径和 CLI 平台包扫描逻辑，只保留三件 Logger 特有的事情：
