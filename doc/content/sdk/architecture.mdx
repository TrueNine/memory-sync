...
graph TB
    ...
    subgraph Libraries["Workspace Libraries (Rust-first)"]
        ...
        subgraph L1["Logger<br/>基础设施层"]
            ...
            LOG_PKG["@truenine/memory-sync-sdk<br/>(Logger wrapper)"]
        end
        ...
        subgraph L3["Script-Runtime<br/>运行时层"]
            ...
            SR_PKG["@truenine/memory-sync-sdk<br/>(SR wrapper)"]
        end
    end
...
<Steps>
  ...
  ```bash
  # TypeScript 编译（并行）
  pnpm -F @truenine/memory-sync-sdk run build:ts
  ...
  此阶段产出：
  - `sdk/src/native_logger.rs` → `dist/index.js` + `napi-memory-sync-cli.*.node`
  - `sdk/src/md-compiler/dist/index.js` + `napi-memory-sync-cli.*.node`
  - `sdk/src/libraries/script-runtime/dist/index.js` + `napi-memory-sync-cli.*.node`
